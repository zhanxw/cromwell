...
class DemoDosResolverSpec extends FlatSpec with Matchers with TableDrivenPropertyChecks {
  ...
  lazy val mockCredentials = {
    val mockTokenValue = "mock-token"
    val mockTokenExpires = Date.from(Instant.now().plus(1, ChronoUnit.DAYS))
    val mockAccessToken = new AccessToken(mockTokenValue, mockTokenExpires)
    val mockCredentials = GoogleCredentials.of(mockAccessToken)
    mockCredentials
  }
  ...
  forAll(parseTests) { (description, responseEntity) =>
    it should description in {
      ...
      val resolver = new MockDemoDosResolver(mockCredentials, normalConfig, mockClient)
      ...
      postRequest.getAllHeaders.length should be(1)
      postRequest.getHeaders(HttpHeaders.AUTHORIZATION).length should be(1)
      postRequest.getFirstHeader(HttpHeaders.AUTHORIZATION).getValue should be("Bearer mock-token")
  ...
  forAll(forbiddenTests) { (description, config, exceptionMessage) =>
    it should description in {
      ...
      val resolver = new MockDemoDosResolver(mockCredentials, config, mockClient)
...
object DemoDosResolverSpec {
  ...
  class MockDemoDosResolver(gcsCredentials: Credentials, config: Config, mockClient: MockClient)
    extends DemoDosResolver(gcsCredentials, config) {
