...
import cromwell.backend.google.pipelines.common.api.PipelinesApiRequestFactory.{CreatePipelineParameters, InputOutputParameters}
...
class PipelinesApiAsyncBackendJobExecutionActor(override val standardParams: StandardAsyncExecutionActorParams)
  ...
  private def createNewJob(): Future[ExecutionHandle] = {
    ...
    def generateJesParameters: Future[InputOutputParameters] = Future.fromTry(Try {
      // Inputs
      val jobFileInputs: Set[PipelinesApiFileInput] = generateJesInputs(jobDescriptor)
      val standardFileInputs: Set[PipelinesApiFileInput] = monitoringScript.toSet + cmdInput
      val fileInputParameters: Set[PipelinesApiFileInput] = jobFileInputs ++ standardFileInputs
      
      val jobFileOutputs = generateJesOutputs(jobDescriptor)
      ...
      val standardFileOutputs = monitoringOutput.toSet + rcFileOutput
      val fileOutputParameters: Set[PipelinesApiFileOutput] = jobFileOutputs ++ standardFileOutputs

      InputOutputParameters(fileInputParameters.toList, fileOutputParameters.toList, gcsAuthParameter.toList)
    })
