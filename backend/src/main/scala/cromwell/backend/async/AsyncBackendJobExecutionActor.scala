...
trait AsyncBackendJobExecutionActor { this: Actor with ActorLogging =>
  ...
  def dockerValueUsed: Option[String]
  ...
  override def receive: Receive = {
    ...
    case Finish(SuccessfulExecutionHandle(outputs, returnCode, jobDetritusFiles, executionEvents, _)) =>
      completionPromise.success(JobSucceededResponse(jobDescriptor.key, Some(returnCode), outputs, Option(jobDetritusFiles), executionEvents, dockerValueUsed))
      context.stop(self)
