...
class JobStoreReaderActor(database: JobStore, override val serviceRegistryActor: ActorRef) extends ReadActor[CommandAndReplyTo[QueryJobCompletion]] with InstrumentedBatchActor[CommandAndReplyTo[QueryJobCompletion]] with ActorLogging with CromwellInstrumentation {
